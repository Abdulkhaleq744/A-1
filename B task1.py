...
y = np.array((X[:, 0] > 0) ^ (X[:, 1] > 0), dtype=int)  # XOR operation
...
# Train MLP classifier with more hidden layers and neurons
mlp = MLPClassifier(hidden_layer_sizes=(20, 20, 20), activation='relu', solver='adam', max_iter=10000, random_state=0)
mlp.fit(X, y)
y_pred = mlp.predict(X)

# Evaluate performance
accuracy = accuracy_score(y, y_pred)
print(f'MLP Classifier Accuracy on XOR Dataset: {accuracy * 100:.2f}%')
