...
class Parameter(Node):
    def __init__(self, value):
        super().__init__()
        self.value = value.astype(np.float64)
        self.gradients = {self: np.zeros_like(self.value, dtype=np.float64)}  # Initialize gradients
    ...
    def backward(self):
        self.gradients[self] = np.zeros_like(self.value, dtype=np.float64)
        for n in self.outputs:
            if self in n.gradients:
                self.gradients[self] += n.gradients[self].astype(np.float64)
# Linear Node
class Linear(Node):
    def __init__(self, input_dim, output_dim, x):
        self.A = Parameter(np.random.randn(output_dim, input_dim) * np.sqrt(2.0 / input_dim))
...
class CrossEntropyLoss(Node):
    ...
    def forward(self):
        ...
        self.value = -np.sum(y_true.value * np.log(y_pred.value + 1e-9)).astype(np.float64)

    def backward(self):
        y_true, y_pred = self.inputs
        self.gradients[y_pred] = ((y_pred.value - y_true.value) / y_true.value.shape[0]).astype(np.float64)
        self.gradients[y_true] = np.zeros_like(y_true.value)  # Ensure y_true gradient is zero
